...
<body class="bg-gray-50 min-h-screen">
  <div class="max-w-4xl mx-auto p-6">
    ...
    <header class="text-center mb-8 bg-white rounded-lg shadow-sm p-6">
      <div class="medical-logo mx-auto mb-4">
        <img src="tm_center_logo.png" alt="TM Center Logo" class="w-full h-full object-contain rounded-full">
      </div>
  ...
  <script>
    // Global
    let quill;
    const STORAGE_KEY = 'medical_templates_v3';
      ...
      templates.forEach(t => {
        ...
        item.querySelector('.delete-template').addEventListener('click', () => {
          if (confirm(`ნამდვილად გსურთ შაბლონის წაშლა: "${t.name}"?`)) {
            deleteTemplate(t.id);
            renderTemplatesInModal(); // განაახლე მოდალი
    ...
    function deleteTemplate(id) {
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify(templates)); // განაახლე localStorage
    }
    ...
    function saveTemplate() {
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify(templates)); // განაახლე
      ...
      const printContent = `
        ...
        <head>
          ...
          <style>
            ...
            .logo { width: 60px; height: 60px; margin: 0 auto 10px; border-radius: 50%; }
            .field { margin: 12px 0; }
